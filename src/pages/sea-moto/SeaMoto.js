...
const SeaMoto = () => {
  ...
  const filteredTours = useMemo(() => {
    ...
    return allEvents.filter(tour => {
      ...
      const isHikingActivity = tour.category && 
        tour.category.name?.toLowerCase() === 'seamoto';
  ...
  const handleBookingSubmit = (e) => {
    ...
    alert(`დაჯავშნა მიღებულია! ${bookingForm.name} - ${bookingDialog.tour.name}`);
  ...
  return (
    <Box>
      ...
      <Dialog 
        ...
        onClose={() => setBookingDialog({ open: false, tour: null })}
        maxWidth="sm"
        fullWidth
      >
        <DialogTitle>
          დაჯავშნა - {bookingDialog.tour?.name}
        </DialogTitle>
        <DialogContent>
          {detailsLoading ? (
          ...
          <Box component="form" onSubmit={handleBookingSubmit} sx={{ mt: 2 }}>
            ...
            <TextField
              ...
              onChange={handleBookingChange}
              margin="normal"
              inputProps={{ min: 1, max: bookingDialog.tour?.maxGroup || 10 }}
              required
            />
          </Box>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setBookingDialog({ open: false, tour: null })}>
            გაუქმება
          </Button>
