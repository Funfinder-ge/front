  ...
  const filteredTours = useMemo(() => {
    ...
    return allEvents.filter(tour => {
      ...
      const isHikingActivity = tour.category && 
        tour.category.name?.toLowerCase() === 'quadro tours';
  ...
  return (
    <Box sx={{margin:"65px 0 50px 0px" }}>
      <Box sx={{ mb: { xs: 2, sm: 4 }, width: "100%", overflow: "hidden" }}>
        <RecentEventsSlider categoryName="quadro tours" />
      </Box>
      ...
      {!eventsLoading && !eventsError && (
        <Box mb={3}>
          ...
          Found {filteredTours.length} Event          </Typography>
        </Box>
      )}
     {/* Tours Grid */}
      {!eventsLoading && !eventsError && (
        ...
        {filteredTours.map((tour) => (
          <Grid item xs={12} sm={6} md={4} key={tour.id} sx={{ display: 'flex', justifyContent: 'center' }}>
            <Card 
              sx={{ 
            ...
            >
              <CardMedia
                component="img"
                height="200"
                image={getImageUrl(tour.primary_image?.image || tour.images?.[0]?.image || tour.image)}
                alt={tour.name}
                sx={{ position: 'relative' }}
              />
              <Box sx={{ position: 'absolute', top: 8, right: 8 }}>
                <Tooltip title={favorites.includes(tour.id) ? 'Remove from favorites' : 'Add to favorites'}>
                  <IconButton
                    onClick={(e) => {
                      e.stopPropagation();
              ...
              <CardContent sx={{ p: 3 }}>
                ...
                <Box display="flex" alignItems="center" mb={1}>
                  ...
                  <Typography variant="body2" color="text.secondary">
                    {tour.city?.name || tour.location || 'Location not specified'}
                  </Typography>
                </Box>
                ...
                <Box display="flex" alignItems="center" mb={2}>
                  <Rating value={tour.average_rating || 0} precision={0.1} size="small" readOnly />
                ...
                <Box display="flex" alignItems="center" justifyContent="space-between" mb={2}>
                  <PriceDisplay
                    ...
                    pricePerPerson={tour.price_per_person || tour.discounted_price}
                    variant="h6"
                  />
